...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!position) {
      return NextResponse.json({ success: false, message: "Staking position not found" }, { status: 404 })
    }
    ...
    return NextResponse.json({ success: false, message: "Failed to unstake position" }, { status: 500 })
  }
}
